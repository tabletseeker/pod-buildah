...
## Roadmap
- [ ] [Features](https://github.com/tabletseeker/pod-test#features)
- [ ] [Sneak Peek](https://github.com/tabletseeker/pod-test/blob/main/README.md#preview)
- [ ] [Install Dependencies](https://github.com/tabletseeker/pod-test/blob/main/README.md#installation)
- [ ] [How it works](https://github.com/tabletseeker/pod-test#how-it-works)
    - [ ] [Basics](https://github.com/tabletseeker/pod-test#mechanics)
    - [ ] [Tmux Shortcuts](https://github.com/tabletseeker/pod-test#session-management)
    - [ ] [Tmux Sessions](https://github.com/tabletseeker/pod-test#session-management)
    - [ ] [Exit Status](https://github.com/tabletseeker/pod-test#container-exit)
    - [ ] [Build Folders](https://github.com/tabletseeker/pod-test#build-folders)
- [ ] [Getting Started](https://github.com/tabletseeker/pod-test#getting-started)
    - [ ] [First Steps](https://github.com/tabletseeker/pod-test#getting-started)
    - [ ] [Command Options](https://github.com/tabletseeker/pod-test#command-options)
    - [ ] [Parameter Description](https://github.com/tabletseeker/pod-test#options-in-depth)
    - [ ] [Command Structure](https://github.com/tabletseeker/pod-test#command-examples)
    - [ ] [Folder Structure](https://github.com/tabletseeker/pod-test#file-overview)
    - [ ] [Volume Mounts](https://github.com/tabletseeker/pod-test#mount-directories)
    - [ ] [Default Behavior](https://github.com/tabletseeker/pod-test#container-defaults)
    - [ ] [Environment Variables](https://github.com/tabletseeker/pod-test#container-environment)
    - [ ] [Systemd Integration](https://github.com/tabletseeker/pod-test#using-services-for-automation)
    - [ ] [Apt-Cacher-Ng](https://github.com/tabletseeker/pod-test#caching)
...
* Once a container exits, because execution of its main process has been completed (main script `pod_start` exits), the exit status will be indicated graphically in the target container's tmux pane. Option `-tc|--tmux-command` allows for a custom post command to be run inside tmux by pressing `Enter` when this message appears. Option `-pc|--post-command` allows for a custom command to be executed inside the container after the container's main process finishes. See [🔼 Parameter Description](#Options-In-Depth)
...
* The same applies when a custom command is run inside a container.
  * Running a command chain inside container linux_kernel that exits 0
    ```sh
    ./pod-buildah -t linux_kernel -c "/bin/bash -c 'sudo apt update && cat /etc/*-release'"
    ```
    <img src="https://i.postimg.cc/BZF0Dg7K/7a.png" width="85%" height="85%">

  * If any part of the chain exits 1, the red bar will appear.
    ```sh
    ./pod-buildah -t linux_kernel -c "/bin/bash -c 'sudo apt update && cat /etc/*-release && exit 1'"
    ```
...
2. Or multiple targets can be run in sequence, meaning the specified arguments will apply to all given target containers equally.
  ...
  ```
3. When running singular instances of pod-buildah in a command chain, make sure to use option `-d|--detach` so that the tmux session isn't attached until the final container is started.
  ```sh
...
* Executes a custom command chain in the container linux_kernel
  ```sh
  ./pod-buildah -t linux_kernel -c "/bin/bash -c 'whoami;sleep 5; cat /etc/*-release'"
  ```
* Executes a custom command chain in containers linux_kernel and nyxt
  ```sh
  ./pod-buildah -t nyxt,linux_kernel -c "/bin/bash -c 'sudo apt update && sudo apt install htop && htop'"
  ```
